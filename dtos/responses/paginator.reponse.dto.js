module.exports = {
    build(currentItemsCount, currentPage, pageSize, totalResourcesCount, basePath, order='createdAt', orderMethod='DESC'){
        const data = {};
        ...
        data.next_page_url = `${basePath}?page=${data.next_page_number}&page_size=${data.requested_page_size}&order=${order}&orderMethod=${orderMethod}`;
        data.prev_page_number = `${basePath}?page=${data.prev_page_number}&page_size=${data.requested_page_size}&order=${order}&orderMethod=${orderMethod}`;
